...
export async function deleteQuote(id: string) {
  try {
    ...
    const { error } = await supabase.from("quotes").delete().eq("id", id).eq("userId", session.userId)
...
export async function resetPassword(token: string, newPassword: string) {
  ...
  try {
    ...
    if (verifyError) {
      ...
      return { error: "Failed to verify reset token. The link may have expired or is invalid." }
    }
